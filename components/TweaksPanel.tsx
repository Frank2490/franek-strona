...
const FONTS: Record<string, { fam: string; w: number }> = {
  ...
  'Bricolage Grotesque':      { fam: "'Bricolage Grotesque', sans-serif",          w: 700 },
  'Instrument Serif':         { fam: "'Instrument Serif', Georgia, serif",          w: 400 },
}
...
function applyTweaks(t: TweakValues) {
  ...
  document.body.dataset.font = t.displayFont.replace(/\s+/g, '-')
}

function useTweaks(defaults: TweakValues): [TweakValues, (key: string, val: string) => void] {
...
function TweakRadio({ label, value, options, onChange }: { label: string; value: string; options: string[]; onChange: (v: string) => void }) {
  ...
  const maxPerCount: Record<number, number> = { 2: 16, 3: 10 }
  const fitsAsSegments = maxLen <= (maxPerCount[options.length] ?? 0)
        ...
        {options.map((o, i) => {
          ...
          return (
            <button key={i} type="button" className="twk-chip" role="radio"
                    ...
                    style={{ background: o }}
                    onClick={() => onChange(o)}>
              {on && <TwkCheck light={__twkIsLight(o)} />}
            </button>
          )
        })}
      </div>
...
// ── Main TweaksApp exported to page ─────────────────────────────────────────

export default function TweaksPanelApp() {
  const [t, setTweak] = useTweaks(TWEAK_DEFAULTS)

  React.useEffect(() => { applyTweaks(t) }, [t])

  return (
    <TweaksPanel title="Tweaks — Atelier">
      ...
      <TweakColor
        label="Akcent"
        value={t.accent}
        options={['#C2613C', '#1a1a2e', '#3F66B0', '#8A4F86', '#B8862F']}
      ...
      <TweakRadio
        label="Styl"
        value={t.bg}
        options={['mesh', 'kropki', 'linie', 'brak']}
